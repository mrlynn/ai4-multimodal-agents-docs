...
This is a complete reference walkthrough with all solutions included. Perfect for solo learners who want to see the full implementation. Work at your own pace and don't hesitate to experiment with the code!
:::
...
# ✅ Checkpoint: Verify MongoDB connection
assert result.get("ok") == 1, "❌ MongoDB connection failed. Check your connection string in .env"
...
# ✅ Checkpoint: Verify PDF loaded
assert pdf.page_count > 0, "❌ PDF has no pages. Check the download URL."
print(f"✅ PDF loaded successfully with {pdf.page_count} pages!")
```
...
<QuickCheck
  question="What's the main advantage of processing PDFs from streams vs saving temporary files?"
  options={[
    "Better security and memory efficiency",
    "Faster processing speed",
    "Better image quality"
  ]}
  correctAnswer={0}
  explanation="Stream processing avoids temporary files and reduces security risks while improving memory efficiency."
/>

<QuickCheck
  question="True or False: PyMuPDF automatically detects file types from streams."
  options={[
    "True - auto-detection works",
    "False - filetype parameter required"
  ]}
  correctAnswer={1}
  explanation="When using streams, you must specify the filetype parameter for proper parsing."
/>
...
# ✅ Checkpoint: Verify image extraction
assert len(docs) > 0, "❌ No images were extracted. Check the PDF processing."
print(f"✅ Successfully extracted {len(docs)} page images!")
```
...
<SlideRecap 
  title="Checkpoint Recap: Document Processing Complete"
  items={[
    { icon: "📄", title: "PDF Downloaded", description: "DeepSeek research paper loaded from arXiv successfully." },
    { icon: "🖼️", title: "Images Extracted", description: "All PDF pages converted to high-resolution PNG images." },
    { icon: "📏", title: "Metadata Captured", description: "Image dimensions and file paths stored for database." },
    { icon: "✅", title: "Ready for Embeddings", description: "Document processing pipeline complete and verified." }
  ]}
  nextSection="Up next: Ingesting embeddings into MongoDB!"
/>
...
# ✅ Checkpoint: Verify data ingestion
assert doc_count > 0, "❌ No documents were ingested. Check your embeddings.json file."
print("✅ All embeddings successfully stored in MongoDB!")
```
...
<QuickCheck
  question="Why is insert_many() preferred over multiple insert_one() calls?"
  options={[
  ...
  correctAnswer={0}
  explanation="Bulk operations reduce network overhead and database load, significantly improving performance."
/>

<QuickCheck
  question="True or False: MongoDB requires you to create databases and collections before inserting data."
  options={[
    "True - must be created explicitly",
    "False - created automatically on first insert"
  ]}
  correctAnswer={1}
  explanation="MongoDB uses lazy creation, automatically creating databases and collections when needed."
/>

<SlideRecap 
  title="Checkpoint Recap: Data Foundation Ready"
  items={[
    { icon: "📦", title: "Embeddings Loaded", description: "Pre-generated 1024-dimensional vectors from embeddings.json." },
    { icon: "🗃️", title: "Collection Created", description: "MongoDB collection with all document metadata and embeddings." },
    { icon: "🔢", title: "Data Verified", description: "Document count confirmed - ready for vector indexing." },
    { icon: "🚀", title: "Search Foundation", description: "Everything in place to build vector search capabilities." }
  ]}
...
# ✅ Checkpoint: Verify index creation started
...
<QuickCheck
  question="Which similarity metric works best for normalized embedding vectors?"
  options={[
  ...
  correctAnswer={0}
  explanation="Cosine similarity is perfect for normalized embeddings as it measures angle, not magnitude differences."
/>

<QuickCheck
  question="True or False: Vector index creation is synchronous and blocks your application."
  options={[
    "True - must wait for completion",
    "False - happens asynchronously in background"
  ]}
  correctAnswer={1}
  explanation="Atlas builds indexes asynchronously, allowing your application to continue running while indexing happens."
/>
...
def get_information_for_question_answering(user_query: str) -> List[str]:
    ...
    # ✅ Checkpoint: Verify search results
    assert len(results) > 0, "❌ No search results found. Check if your vector index is READY."
...
<SlideRecap 
  ...
  items={[
    { icon: "📇", title: "Index Created", description: "1024-dimensional vector index with cosine similarity ready." },
    { icon: "🔍", title: "Search Function Built", description: "Query embedding and aggregation pipeline working correctly." },
    { icon: "⚙️", title: "Function Declaration", description: "Gemini can now understand how to use your search tool." },
    { icon: "🧠", title: "RAG Foundation", description: "Retrieval-Augmented Generation system ready for AI agent." }
  ]}
...
<QuickCheck
  question="What does temperature=0.0 achieve in LLM generation?"
  options={[
  ...
  correctAnswer={0}
  explanation="Temperature 0.0 removes randomness for consistent, fact-based answers ideal for factual tasks."
/>
...
<QuickCheck
  ...
  options={[
    ...
    "False - the LLM decides when based on context"
  ]}
  correctAnswer={1}
  explanation="Function declarations provide the schema, but the LLM autonomously decides when to use functions based on context."
/>
...
# ✅ Checkpoint: Verify index creation
print("✅ Session index created for efficient history queries!")
```
...
def store_chat_message(session_id: str, role: str, type: str, content: str) -> None:
    ...
    print(f"💾 Stored {type}-{role} message for session {session_id}")
...
<QuickCheck
  ...
  correctAnswer={0}
  explanation="Session IDs create isolated conversation threads, perfect for multi-user applications."
/>
...
<SlideRecap 
  ...
  items={[
    { icon: "🧠", title: "Memory System Complete", description: "Session-based conversation history with MongoDB storage." },
    { icon: "🔄", title: "Multi-turn Conversations", description: "Agent remembers context across multiple interactions." },
    { icon: "📋", title: "Context Management", description: "Seamlessly includes conversation history in LLM input." },
    { icon: "✅", title: "Production Ready", description: "Scalable, stateful conversational AI system complete!" }
  ]}
...
<QuickCheck
  question="What's the main advantage of ReAct agents over simple function calling?"
  options={[
    "They can reason about whether they have sufficient information",
    "They are faster to execute",
    "They use less memory"
  ]}
  correctAnswer={0}
  explanation="ReAct agents can decide 'do I have enough context?' before answering, leading to more thoughtful responses."
/>

<QuickCheck
  question="True or False: ReAct agents always make better decisions than simple agents."
  options={[
    "True - always superior",
    "False - depends on the use case and implementation"
  ]}
  correctAnswer={1}
  explanation="ReAct adds complexity and cost; simple agents may be better for straightforward tasks where reasoning isn't needed."
/>
...
# ✅ Checkpoint: Verify memory functionality
...
<SlideRecap 
  title="Workshop Complete: Advanced Multimodal Agent System!"
  items={[
    { icon: "📄", title: "PDF Processing", description: "Complete pipeline from documents to searchable embeddings." },
    { icon: "🔍", title: "Vector Search", description: "MongoDB Atlas integration with intelligent similarity search." },
    { icon: "🤖", title: "AI Agent", description: "Gemini 2.0 Flash with function calling and multimodal understanding." },
    { icon: "🧠", title: "Advanced Reasoning", description: "ReAct pattern for sophisticated decision-making and tool usage." }
  ]}
...
**Navigation:** **[← Complete Notebook Reference](./complete-notebook)** | **[🏠 Workshop Overview](./index)** | **[🚀 Next Steps & Resources](./next-steps)**
