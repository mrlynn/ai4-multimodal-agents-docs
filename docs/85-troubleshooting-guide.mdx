...
**"Divide and conquer"** - Isolate issues by testing each component separately: PDF processing → Embedding generation → Database storage → AI responses.
:::
...
| **MongoDB** | Test connection in n8n | ✅ Connection successful | Check credentials/network |
...
# Go to GitHub Codespaces dashboard → Delete failed codespace → Create new one

# Method 2: Use different machine type
# Select: 4-core • 8 GB RAM • 32 GB storage (instead of 2-core)
...
async function testConnection() {
  try {
    ...
    if (error.code === 8000) {
      ...
      console.log('🔍 Network issue - check IP allowlist');
    } else if (error.message.includes('ENOTFOUND')) {
      console.log('🔍 DNS issue - check connection string');
    }
  }
}
...
async function testVoyageAPI() {
  ...
  if (!apiKey.startsWith('pa-')) {
    console.error('❌ Invalid API key format (should start with "pa-")');
    return;
  }
...
2. **GitHub Issues**: [Repository Issues](https://github.com/mrlynn/ai4-multimodal-agents-n8n/issues)
